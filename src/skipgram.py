...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        # TODO
        noise_words: torch.Tensor = torch.multinomial(noise_dist, n_samples*batch_size)

        device: str = "cuda" if self.out_embed.weight.is_cuda else "cpu"
        noise_words: torch.Tensor = noise_words.to(device)

        # Reshape output vectors to size (batch_size, n_samples, n_embed) (10,5,50)
        # TODO
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # z =  torch.bmm(input_vectors, output_vectors)
        # out_loss = torch.sum(torch.log(torch.sigmoid(z)), dim=1)

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_loss = torch.bmm(noise_vectors.neg(), input_vectors).sigmoid().log()
        noise_loss = noise_loss.squeeze().sum(1)
        # noise_z = torch.bmm(noise_vectors, input_vectors)
        # noise_loss = torch.sum(torch.log(torch.sigmoid(-noise_z)), dim=1)

        # Return the negative sum of the correct and noisy log-sigmoid losses, averaged over the batch
        # TODO
        loss = -(noise_loss + out_loss).mean()
        return loss